...
| **MyAnimeList** | `aniLink.mal`     | `anime.get`, `manga.get`, and `user.me` REST reads with field selection, the `seasonal`, `ranking`, and `suggestions` discovery reads, plus `anime`/`manga` `updateMyListStatus` and `deleteFromList` list-status writes |
