...
/**
 * Keys selected in every composed media document: the entity handles callers
 * need to follow up with any other call.
 */
...
export class MediaQuery extends AniListOperation {
    ...
    async media<K extends FieldPath<MediaResponse>>(
        variables: MediaVariables,
        options: RequestOptions & { fields: readonly K[] | undefined }
    ): Promise<DeepPick<MediaResponse, K | "id" | "idMal">>;
    async media(
        variables: MediaVariables,
        options?: RequestOptions & FieldsSelection<MediaResponse>
    ): FieldsResult<MediaResponse, "id" | "idMal"> {
        const query = `
    ...
    `;
        ...
        return await this.execute<MediaResponse>(
