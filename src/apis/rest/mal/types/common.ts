 ...
 *
 * It is the `main_picture` shape inside `MalAnime` and `MalManga` and is selected via {@link MalRequestOptions.fields} through `MalAnimeOperation.get`, `MalMangaOperation.get`, `MyAnimeListAnimeApi.get`, and `MyAnimeListMangaApi.get`.
 *
 ...
 *
 * It extends {@link RequestOptions} with the MyAnimeList `fields` selector: every read and list-status write through `MyAnimeListApi` sends it as the `fields` query parameter, while the list-status deletes accept and ignore it. Transport settings are merged over the instance defaults from `MalCredentials` via `buildMyAnimeListApi`.
 *
 * @see https://myanimelist.net/apiconfig/references/api/v2#tag/anime/operation/anime_anime_id_get
