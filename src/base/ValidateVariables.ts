    ...
    for (const [variable, value] of Object.entries(variables)) {
        ...
        if (expectedType) {
            if (Array.isArray(expectedType)) {
                ...
                if (Array.isArray(value)) {
                    value.forEach((item, index) => {
                        if (!expectedType.includes(item)) {
                            errors.push(
                                `Invalid ${variable}[${index}]: ${item}. Expected one of: ${expectedType.join(", ")}`
                            );
                        }
                    });
                } else if (!expectedType.includes(value)) {
                    errors.push(
                        `Invalid ${variable}: ${value}. Expected one of: ${expectedType.join(", ")}`
                    );
                }
                ...
                if (
                    !Array.isArray(value) ||
                    !value.every((element: any) => typeof element === elementType)
                ) {
                    ...
                    value.forEach((item, index) => {
                        for (const [prop, propValue] of Object.entries(item as object)) {
                            const expectedPropType =
                                expectedType[prop as keyof typeof expectedType];
                            if (expectedPropType) {
                                if (
                                    expectedPropType === "boolean" &&
                                    ...
                                    errors.push(
                                        `Invalid ${variable}[${index}].${prop}: ${propValue}. Expected one of: ${expectedPropTypeString}`
                                    );
                                }
                            }
                        }
                        ...
                        if (expectedPropType) {
                            if (Array.isArray(propValue) && propValue.length === 1) {
                                ...
                                if (
                                    ...
                                    expectedPropType.includes(propValue[0])
                                ) {
                                    ...
                                    errors.push(
                                        `Invalid ${variable}.${prop}: ${propValue[0]}. Expected type: ${expectedPropType}`
                                    );
                                }
                                ...
                                if (!propValue.every((item) => expectedPropType.includes(item))) {
                                    ...
                                    errors.push(
                                        `Invalid ${variable}.${prop}: ${propValue}. Expected type: ${expectedPropTypeString}`
                                    );
                                }
                            } else if (
                                typeof propValue === "string" &&
                                expectedPropType.includes(propValue)
                            ) {
                ...
                value.forEach((item, index) => {
                    if (typeof item !== expectedType) {
                        errors.push(
                            `Invalid ${variable}[${index}]: ${item}. Expected type: ${expectedType}`
                        );
                    }
                });
