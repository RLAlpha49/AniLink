...
import type { ComparisonResult, Discrepancy, PackageOperation, Schema, SelectionNode } from './types'
...
function compareArguments(operation: PackageOperation, field: IntrospectionField, discrepancies: Discrepancy[]): void {
  const schemaArguments = new Map(field.args.map((argument) => [argument.name, argument]))
  for (const argument of operation.arguments) {
    if (!schemaArguments.has(argument)) {
      discrepancies.push({ severity: 'error', category: 'missing-argument', operation: operation.exportName, sourcePath: operation.sourcePath, packageValue: argument, message: `Argument ${argument} is not present on ${operation.rootField}` })
    }
  }
  for (const variable of operation.variables) {
    ...
    if (apiType !== variable.type) {
      discrepancies.push({ severity: 'error', category: 'variable-type-mismatch', operation: operation.exportName, sourcePath: operation.sourcePath, packageValue: variable.type, apiValue: apiType, message: `Variable ${variable.name} has type ${variable.type}; AniList expects ${apiType}` })
    }
  }
}

function compareSelection(operation: PackageOperation, selection: SelectionNode[], typeRef: IntrospectionTypeRef, types: Map<string, Schema['__schema']['types'][number]>, discrepancies: Discrepancy[]): void {
  const objectType = unwrapObject(typeRef, types)
  if (!objectType) return
  for (const node of selection) {
    const field = getField(objectType, node.name)
    if (!field) {
      discrepancies.push({ severity: 'error', category: 'missing-response-field', operation: operation.exportName, sourcePath: operation.sourcePath, packageValue: node.name, message: `Response field ${node.name} is not present on ${objectType.name}` })
      continue
    }
...
function unwrapObject(typeRef: IntrospectionTypeRef, types: Map<string, Schema['__schema']['types'][number]>): IntrospectionObjectType | undefined {
...
function getField(type: Schema['__schema']['types'][number] | undefined, name: string): IntrospectionField | undefined {
  return type?.kind === 'OBJECT' || type?.kind === 'INTERFACE' ? type.fields?.find((field) => field.name === name) : undefined
}
