...
export function renderMarkdown(result: ComparisonResult, metadata: ReportMetadata): string {
  ...
  for (const discrepancy of result.discrepancies) {
    ...
    if (discrepancy.packageValue !== undefined) lines.push(`- Package value: \`${JSON.stringify(discrepancy.packageValue)}\``)
    if (discrepancy.apiValue !== undefined) lines.push(`- API value: \`${JSON.stringify(discrepancy.apiValue)}\``)
    lines.push('')
  }
  return `${lines.join('\n')}\n`
}
