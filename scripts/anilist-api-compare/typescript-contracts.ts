...
function extractMembers(
  ...
  return Object.fromEntries(
    members
      ...
      .map((member) => {
        const propertyName = member.name && ts.isIdentifier(member.name) ? member.name.text : undefined
        if (!propertyName || !member.type) return undefined
        return [propertyName, { ...normalizeType(member.type, warnings, sourcePath, propertyName), optional: Boolean(member.questionToken) }]
      })
...
function normalizeType(
  ...
  if (ts.isArrayTypeNode(type)) {
    return { ...normalizeType(type.elementType, warnings, sourcePath, propertyName), array: true }
  }
  ...
  if (ts.isTypeReferenceNode(type)) return { type: type.typeName.getText(), optional: false, array: false }
