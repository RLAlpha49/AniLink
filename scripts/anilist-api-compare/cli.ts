...
import { discoverPackageOperations } from './package-inventory'
...
export async function runCli(options: CliOptions): Promise<CliResult> {
  ...
  try {
    log(options.argv[0] === 'update-schema' ? 'AniList schema snapshot update started' : 'AniList API comparison started')
    ...
    log(result.discrepancies.length ? 'Actionable discrepancies require review' : 'No discrepancies found')
    log('Reports: artifacts/anilist-api-compare/report.md, artifacts/anilist-api-compare/report.json')
...
async function runComparison(argv: string[]): Promise<ComparisonResult> {
  ...
  const schemaPath = resolve(root, valueAfter(argv, '--schema') ?? 'scripts/anilist-api-compare/anilist-schema.json')
  const reportDirectory = resolve(root, valueAfter(argv, '--report-dir') ?? 'artifacts/anilist-api-compare')
  const schema = live ? await fetchSchema() : await loadSchema(schemaPath)
  const operations = await discoverPackageOperations(resolve(root, 'src/apis/anilist'))
  const result = comparePackageToSchema({ schema, operations })
  await mkdir(reportDirectory, { recursive: true })
  await writeFile(resolve(reportDirectory, 'report.md'), renderMarkdown(result, { schemaSource: live ? 'live' : schemaPath }), 'utf8')
  await writeFile(resolve(reportDirectory, 'report.json'), renderJson(result, { schemaSource: live ? 'live' : schemaPath }), 'utf8')
  return result
}

async function updateSchema(): Promise<void> {
  const schema = await fetchSchema()
  await writeSchema(resolve(process.cwd(), 'scripts/anilist-api-compare/anilist-schema.json'), schema)
}
