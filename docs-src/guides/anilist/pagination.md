...
| `onHookError`              | `paginate`/`paginateChunks` | —       | —     | Observer for failures thrown by `onPage`/`onChunk`. When provided, a throwing callback is reported to this handler with the hook name and the error instead of falling back to `console.warn`                                                                                                                                                                 |
