...
| `onPace`              | `OnPaceHandler`                   | —               | Fires after a rate-limit pacing wait completes, before the request is dispatched (an aborted wait emits nothing — observe it via `onError` with `abortedDuringPacing: true`)                         | Pacing telemetry                                    |
