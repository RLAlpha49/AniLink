...
| `onPace`              | `OnPaceHandler`                   | none            | Fires after a rate-limit pacing wait completes, before the client sends the request. An aborted wait emits the elapsed portion with `aborted: true`                                                  | Pacing telemetry                                    |
