...
Builds an AniList `FuzzyDateInput` from optional year, month, and day parts. AniList represents unknown fuzzy-date parts as `0`; this helper fills each omitted part with `0`, so the result always satisfies the `FuzzyDateInput` contract. Use it to construct `startedAt`/`completedAt` values for list-entry mutations — no hand-built objects, no guessing which fields to zero out.
...
`mediaListCollection` returns lists nested by status and custom list — a maze of groups. `flattenMediaListCollection` flattens that structure into a single array of `FlattenedMediaListEntry` objects, deduplicated by entry id.
