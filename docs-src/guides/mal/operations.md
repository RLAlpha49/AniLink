...
Gets one manga by its MyAnimeList ID. Calls `GET /manga/{id}` on the MAL API v2.
...
**Returns:** `MalMangaListStatus` — the updated list status. MAL reports the chapter count as `num_chapters_read` and returns `tags` as a single comma-separated string.
...
Removes a manga from the authenticated user's list. Calls `DELETE /manga/{id}/my_list_status`.
