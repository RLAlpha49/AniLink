...
- **GraphQL mutations** (AniList writes) drop every cached GraphQL query at the endpoint. The cache keys every operation of one provider at the same URL, and a mutation can change what many different query documents return. A `SaveMediaListEntry`, for example, changes what both a `MediaList` and a `MediaListCollection` query report. The cache cannot derive a finer-grained prefix than the endpoint. This is deliberately conservative. The next reads refetch fresh data instead of serving pre-mutation entries.
