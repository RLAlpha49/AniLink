...
| `onPace`              | `OnPaceHandler`                     | instance value         | Fires after a rate-limit pacing wait completes, before the client dispatches the request; an aborted wait emits the elapsed portion with `aborted: true` (see [Observability](/observability)).                                                                                                                                                    |
