 ...
 *
 * The inline head script in config.mts defines the `--rd-*` design tokens
 * globally on `:root` / `html.dark` before first paint, so they cascade
 * everywhere, including teleported content, without this composable.
 * This is only for code that needs the theme as a *value*. It reads the
 * active theme from the same source `DocsLayout` uses
