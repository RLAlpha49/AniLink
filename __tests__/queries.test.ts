...
const queryCases: Array<[string, string, object | undefined, string]> = [
  ...
  ['media list collection', 'mediaListCollection', { userId: 542244, type: 'ANIME', status: 'COMPLETED', chunk: 1, perChunk: 100 }, 'MediaListCollection'],
  ...
  ['external link source collection', 'externalLinkSourceCollection', undefined, 'ExternalLinkSourceCollection']
]

describe('AniList single-resource queries', () => {
  test.each(queryCases)('%s is handled without network access', async (_name, method, variables, operation) => {
